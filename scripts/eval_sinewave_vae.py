...
import numpy as np
import pandas as pd
from cmmr25_isudt.models.models import PlSineFactorVAE
from cmmr25_isudt.utils.misc import midi2frequency, frequency2midi
from cmmr25_isudt.utils.tensor import scale
...
if num_pairs > 0:
    ...
    fig.suptitle(f"Sound Model v{model_version} | Epoch {epoch_idx} | Latent Space")
    ...
    plt.savefig("../figures/sound_model_latent_space.png", dpi=300)
...
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig("../figures/sound_samples.png", dpi=300)
# Reset rcParams to default to not affect other plots
plt.rcdefaults()
...
plt.savefig("../figures/sound_model_traverse_latent_space_dim1.png")
...
# remove white background
fig.patch.set_visible(False)
plt.savefig("../figures/sound_model_traverse_latent_space_dim0.png")
